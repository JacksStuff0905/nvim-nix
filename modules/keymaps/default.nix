...
in
{
  ...
  config.vim.keymaps = lib.mkIf cfg.enable (
    lib.mkMerge [
      # Configure window navigation
      (lib.mkIf cfg.window-navigation.enable [
        {
          key = cfg.window-navigation.left;
          mode = "n";
          action = "<C-w>h";
        }
        {
          key = cfg.window-navigation.down;
          mode = "n";
          action = "<C-w>j";
        }
        {
          key = cfg.window-navigation.up;
          mode = "n";
          action = "<C-w>k";
        }
        {
          key = cfg.window-navigation.right;
          mode = "n";
          action = "<C-w>l";
        }

        {
          key = cfg.window-navigation.previous;
          mode = "n";
          action = "<C-o>";
        }
        {
          key = cfg.window-navigation.next;
          mode = "n";
          action = "<C-i>";
        }
      ])
      ...
      (lib.mkIf cfg.search-highlighting.enable [
        {
          key = cfg.search-highlighting.key;
          mode = "n";
          action = ":noh<CR>";
        }
      ])
